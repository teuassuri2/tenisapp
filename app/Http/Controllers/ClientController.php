...
class ClientController extends Controller {
    ...
    public function storeApi(StoreClientRequest $request) {
        if ($request->isMethod("post")) {
            $client = $this->clientService->store($request->validated());
            return response()->json(new ClientJsonResource($client), 200);
        }
    }

    public function editApi(Client $client, StoreClientRequest $request) {
        if ($request->isMethod("post")) {
    ...
    public function edit(Client $client, StoreClientRequest $request) {
        if ($request->isMethod("post")) {
            $client = $this->clientService->update($client, $request->validated());
        }
