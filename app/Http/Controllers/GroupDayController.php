...
use App\Http\Resources\GroupDayResource;
...
class GroupDayController extends Controller {
    ...
    public function __construct(GroupDayService $groupDayService) {
    ...
    public function storeApi(StoreGroupDayRequest $request) {
        if ($request->isMethod("post")) {
            $groupDay = $this->groupDayService->store($request->validated());
            return response()->json(new GroupDayJsonResource($groupDay), 200);
        }
    }

    public function editApi(GroupDay $groupDay, StoreGroupDayRequest $request) {
        if ($request->isMethod("post")) {
            $groupDay = $this->groupDayService->update($groupDay, $request->validated());
            return response()->json(new GroupDayJsonResource($groupDay), 200);
        }
    }
    ...
    public function edit(GroupDay $groupDay, StoreGroupDayRequest $request) {
        if ($request->isMethod("post")) {
            $groupDay = $this->groupDayService->update($groupDay, $request->validated());
        }
    ...
    public function deleteApi(int $id) {
        if ($request->isMethod("post")) {
            $this->groupDayService->delete($id);
