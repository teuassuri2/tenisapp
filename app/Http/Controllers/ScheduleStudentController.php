...
use App\Http\Resources\ScheduleStudentResource;
...
class ScheduleStudentController extends Controller {
    ...
    public function __construct(ScheduleStudentService $scheduleStudentService) {
    ...
    public function storeApi(StoreScheduleStudentRequest $request) {
        if ($request->isMethod("post")) {
            $scheduleStudent = $this->scheduleStudentService->store($request->validated());
            return response()->json(new ScheduleStudentJsonResource($scheduleStudent), 200);
        }
    }

    public function editApi(ScheduleStudent $scheduleStudent, StoreScheduleStudentRequest $request) {
        if ($request->isMethod("post")) {
            $scheduleStudent = $this->scheduleStudentService->update($scheduleStudent, $request->validated());
            return response()->json(new ScheduleStudentJsonResource($scheduleStudent), 200);
        }
    }

    public function store(StoreScheduleStudentRequest $request) {
        if ($request->isMethod("post")) {
            $scheduleStudent = $this->scheduleStudentService->store($request->validated());
        }
    ...
    public function edit(ScheduleStudent $scheduleStudent, StoreScheduleStudentRequest $request) {
        if ($request->isMethod("post")) {
            $scheduleStudent = $this->scheduleStudentService->update($scheduleStudent, $request->validated());
        }
