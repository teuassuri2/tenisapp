<?php  namespace App\Services; use App\Models\User; class UserService {  private User $user;public function __construct(User $user)
    {
        $this->user = $user;
    }public function store(array $data){ $this->user->name = $data['name'];
 ...
 $this->user->save();
 return $this->user;
}public function update(User $user, array $data){ $user->name = $data['name'];
